...
Formula:
  Price Opportunity      25%
  Future Appreciation    25%
  Developer              20%
  Community              15%
  Liquidity              10%
  ROI (post-handover)     5%
...
def calculate_post_handover_roi(
    ...
    # Expenses: service charge (from DLD Mollak data), management fee (5%), vacancy (5%)
    service_charge_per_sqft = None
    if project_data:
        sc = project_data.get("serviceCharge")
    ...
    if has_service_charge:
        service_charge = size_sqft * service_charge_per_sqft
    else:
        service_charge = None
    ...
    if has_service_charge:
        ...
        net_income = estimated_rent - management_fee - vacancy
    ...
    return {
        ...
        "rentSource": rent_source,
        "serviceChargeAnnual": round(service_charge) if has_service_charge else None,
...
def calculate_developer_score(dev_data: dict | None) -> dict:
    ...
    return {
        ...
        "delayRisk": dev_data.get("deliveryDelayRisk", "Medium"),
        "delayedProjectsPct": safe_float(dev_data.get("delayedProjects")),
        "marketPosition": dev_data.get("marketPosition", "Tier 3"),
...
def score_offplan_property(
    ...
    investment_score = round(
        price_diff["priceOpportunityScore"] * 0.25 +
        future_apprec["futureAppreciationScore"] * 0.25 +
        dev_score["developerScore"] * 0.20 +
        comm_score["communityScore"] * 0.15 +
        liquidity["liquidityScore"] * 0.10 +
        roi["roiScore"] * 0.05
    )
    ...
    overall_risk = round(
        ...
        max(0, price_diff["priceDifferencePct"]) * 2 * 0.25 +
        (100 - future_apprec["futureAppreciationScore"]) * 0.20
    )
    ...
    return {
        ...
        "sizeSqft": size_sqft,
        "askingPrice": price,
        "priceSqft": round(price / size_sqft, 2) if size_sqft > 0 else 0,
        "status": "offplan",
