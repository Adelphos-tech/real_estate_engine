...
CRITICAL RULES:
1. You receive ALL computed metrics. You do NOT recalculate anything. You write the advisory narrative.
2. You MUST reference the investor's actual goal. If goal is Capital Growth, talk about appreciation, NOT rental income.
3. If goal is Capital Growth, do NOT mention rental yield, net income, vacancy, or tenant demand as key factors.
4. If goal is Rental Income, focus on yield, rent, occupancy — NOT appreciation.
5. Your recommendation MUST match the deterministic recommendation provided. If engine says HOLD, you say HOLD. Never contradict.
6. Do NOT invent reasoning. Explain the existing deterministic scores and flags.
7. Do NOT use forbidden words: good, promising, excellent, interesting, potential, caution, safe, amazing, great, opportunity.
8. Exit strategy MUST match the investor's goal:
   - Capital Growth -> Sell after appreciation
   - Rental Income -> Hold and rent
   - Flip -> Sell at handover
   - End User -> Hold for personal use
9. If data is insufficient for a metric, say "insufficient data" — do NOT say "0%" or invent a number.
10. Negotiation tips must reference actual deterministic price data, not invented reasoning.
11. Do NOT use external knowledge about developers, areas, or projects. Only reference the deterministic scores and data provided. If developer score is 35, do NOT say the developer is "known for quality" — say "developer score is 35/100".
12. Your executive summary MUST state the investor's actual goal and whether the property aligns with it.
...
Rule Flags: {flags}"""
    ...
    goal = investor_profile.get("goal", "balanced") if investor_profile else "balanced"
    goal_constraint = f"""
CRITICAL GOAL CONSTRAINT:
- Investor goal is: {goal}
- If goal is capital_growth: Focus on appreciation, growth, area development. Do NOT discuss rental yield, net income, or tenant demand as primary factors. Exit = Sell after appreciation.
- If goal is rental_income: Focus on rental yield, occupancy, cash flow. Exit = Hold and rent.
- If goal is flip_handover: Focus on price discount, liquidity, payment plan. Exit = Sell at handover.
- If goal is end_user: Focus on livability, community, transport. Do NOT discuss ROI or investment returns.
- Deterministic recommendation: {rec} — your advice MUST align with this. Do NOT contradict.
- If growth shows 0%, say "insufficient price history" not "0% growth".
"""

    user_prompt = f"""Write a complete advisory report for this property.

Property: {bed} {property_data.get('category', '')} in {area}
{metrics}{profile_str}{goal_constraint}

Write a professional, client-facing report with these sections:
1. Executive Summary (2 sentences — MUST reference the investor's actual goal)
2. Investment Thesis (3-4 sentences explaining the opportunity or lack thereof FOR THIS GOAL)
3. Key Strengths (2-3 specific points relevant to the investor's goal)
4. Key Risks (2-3 specific points relevant to the investor's goal)
5. Negotiation Tips (1-2 actionable suggestions based on deterministic price data)
6. Exit Strategy (1-2 sentences — MUST match the goal: {goal})
